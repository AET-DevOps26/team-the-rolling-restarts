...
| `docker-compose.azure.yaml` | Used by `deploy-azure` workflow: runs from **pre-built images pulled from ACR** instead of building from source; also clears the base `ollama` service's `local-llm` profile gate so a self-hosted Ollama (model set by the `AZURE_OLLAMA_MODEL` repo variable, default `llama3.2:1b`) runs unconditionally — `logos` is unreachable from Azure's public cloud, so `deploy-azure.yml` hardcodes `LLM_PROVIDER=ollama` there instead of reading the shared variable. Not used by the manual Ansible path — see `docs/source/azure-vm-deployment.md`. |
