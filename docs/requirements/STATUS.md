...
- ⚠️ Pre-commit secret scanning — `.pre-commit-config.yaml`'s `gitleaks` hook blocks commits that introduce secrets (staged-diff scan, same tool/config as above). Only active once a contributor runs `pre-commit install`; nothing enforces that on a fresh clone, and it's bypassable with `--no-verify` — so it's a local guard rail, not a guarantee.
