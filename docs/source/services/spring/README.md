...
- **Outbound SSRF.** content-service validates user-supplied RSS URLs (`UrlSafetyValidator`) against loopback/link-local/site-local/multicast and IPv6 unique-local (`fc00::/7`) ranges, both at source creation and again immediately before each fetch.
