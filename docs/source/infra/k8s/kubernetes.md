...
- [rbac/](rbac) contains `grafana-lgtm-rbac.yml` — a ServiceAccount + namespaced Role/RoleBinding granting `grafana-lgtm`'s Prometheus `get/list/watch` on `pods` only, needed for per-pod metrics scraping.
